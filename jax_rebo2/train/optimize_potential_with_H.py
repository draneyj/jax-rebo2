...
import numpy as np
import optax
import newrebo3_interaction_list as rebo2
from initial_params_CHO import params, make_F_array
import pickle
...
def interaction_loss(params, interactions, box, row, force_weight=1.0, energy_weight=1.0, regularization_weight=0.0):
    species = row['species']
...
if __name__ == "__main__":
    ...
    if 'H' not in params.keys():
        H_zero_true, H_zero_di_true, H_zero_dj_true, H_zero_dk_true = make_F_array(apply_rules=False)
        params['H'] = jnp.array([[H_zero_true for i in range(3)] for j in range(3)])
        params['H_di'] = jnp.array([[H_zero_di_true for i in range(3)] for j in range(3)])
        params['H_dj'] = jnp.array([[H_zero_dj_true for i in range(3)] for j in range(3)])
        params['H_dk'] = jnp.array([[H_zero_dk_true for i in range(3)] for j in range(3)])
